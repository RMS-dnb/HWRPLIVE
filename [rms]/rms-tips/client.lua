...
RegisterNetEvent('rsg-tips:client:addActivityBlip')
AddEventHandler('rsg-tips:client:addActivityBlip', function(data)
...
RegisterNetEvent('rsg-tips:client:removeActivityBlip')
AddEventHandler('rsg-tips:client:removeActivityBlip', function(coords)
...
local function openCriminalActivityMenu()
    if not isLawman() then
        TriggerEvent('ox_lib:notify', { 
            title = 'Access Denied', 
            description = 'Only lawmen can access this menu.', 
            type = 'error' 
        })
        return
    end
    
    lib.callback('rsg-tips:getActivities', false, function(activities)
        ...
        table.insert(menuOptions, {
            ...
            icon = 'user-plus',
            onSelect = function()
                lib.callback('rsg-tips:getOnlinePlayers', false, function(players)
                    ...
                    for _, player in ipairs(players) do
                        table.insert(playerOptions, {
                            ...
                            onSelect = function()
                                ...
                                if input then
                                    TriggerServerEvent('rsg-tips:server:addWantedPlayer', player.serverId, input[1], input[2])
                    ...
                    lib.registerContext({
                        id = 'wanted_player_menu',
                        title = 'Select Player to Add',
                        options = playerOptions
                    })
                    lib.showContext('wanted_player_menu')
                end)
            end
        })
                ...
                table.insert(menuOptions, {
                    ...
                    onSelect = function()
                        local submenuOptions = {
                            {
                                ...
                                icon = 'trash',
                                onSelect = function()
                                    lib.callback('rsg-tips:deleteActivity', false, function(success)
                                        if success then
                                            TriggerEvent('ox_lib:notify', { 
                                                title = 'Success', 
                        ...
                        if coords then
                            table.insert(submenuOptions, {
                                ...
                                onSelect = function()
                                    ...
                                    TriggerEvent('rsg-tips:client:addActivityBlip', blipData)
                                    TriggerEvent('ox_lib:notify', { 
                                        title = 'Map Updated', 
                        ...
                        lib.registerContext({
                            ...
                            title = 'Suspicious Activity in ' .. location,
        ...
        lib.registerContext({
            id = 'criminal_activity_menu',
            title = 'Suspicious Activity Reports',
            options = menuOptions
        })
        lib.showContext('criminal_activity_menu')
    end)
end
...
--     lib.callback('rsg-tips:getWantedPlayers', false, function(wantedPlayers)
...
--                                 icon = 'trash',
--                                 onSelect = function()
--                                     lib.callback('rsg-tips:removeWantedPlayer', false, function(success)
--                                         if success then
...
-- RegisterNetEvent('rsg-tips:client:notifyWanted')
-- AddEventHandler('rsg-tips:client:notifyWanted', function(data)
...
-- RegisterCommand('reportactivity', function(source, args, rawCommand)
--     local playerPed = PlayerPedId()
...
--     local input = lib.inputDialog('Report Suspicious Activity', {
...
--     local description = input[1] and input[1] ~= "" and input[1] or "Suspicious activity observed"
    
    
--     TriggerServerEvent('rsg-tips:server:addCriminalActivity', description, nil, coords)
...
RegisterCommand('activitymenu', function()
    openCriminalActivityMenu()
end, false)

RegisterCommand('bountyboard', function()
    openBountyBoard()
end, false)
