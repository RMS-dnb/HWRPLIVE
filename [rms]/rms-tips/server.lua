...
lib.callback.register('rsg-tips:getOnlinePlayers', function(source)
    local players = {}
...
RegisterNetEvent('rsg-tips:server:addWantedPlayer')
AddEventHandler('rsg-tips:server:addWantedPlayer', function(serverId, crime, bounty)
    local src = source
    ...
    if Target then
        ...
        for _, playerId in ipairs(Players) do
            ...
            if targetPlayer and isLawmanOrBountyHunter(targetPlayer) then
                TriggerClientEvent('rsg-tips:client:notifyWanted', playerId, {
                    charName = charName,
                    crime = crime,
                    bounty = bounty
                })
...
lib.callback.register('rsg-tips:getWantedPlayers', function(source)
...
lib.callback.register('rsg-tips:removeWantedPlayer', function(source, citizenid)
    local Player = RSGCore.Functions.GetPlayer(source)
...
local function addCriminalActivity(activityType, coords)
    ...
    local success, result = safeDatabaseQuery(
        'INSERT INTO criminal_activities (activityType, location, coords) VALUES (?, ?, ?)',
    ...
    for _, playerId in ipairs(Players) do
        ...
        if Player and Config.LawmenJobs[Player.PlayerData.job.name] then
            TriggerClientEvent('ox_lib:notify', playerId, {
                ...
                description = string.format('Suspicious activity reported in %s: %s', location, activityType),
                type = 'inform',
                duration = CONFIG.NOTIFICATION_DURATION
            })
            
            -- Trigger client-side blip creation
            TriggerClientEvent('rsg-tips:client:addActivityBlip', playerId, {
                coords = coords,
                location = location,
...
RegisterNetEvent('rsg-tips:server:addCriminalActivity')
AddEventHandler('rsg-tips:server:addCriminalActivity', function(description, _, coords)
    local src = source
...
lib.callback.register('rsg-tips:getActivities', function(source)
    local activities = MySQL.query.await('SELECT * FROM criminal_activities ORDER BY timestamp DESC')
...
lib.callback.register('rsg-tips:deleteActivity', function(source, activityId)
    local Player = RSGCore.Functions.GetPlayer(source)
    ...
    local existsQuery = MySQL.query.await('SELECT id, coords FROM criminal_activities WHERE id = ?', { activityId })
    ...
    local success, error = pcall(function()
        return MySQL.update.await('DELETE FROM criminal_activities WHERE id = ?', { activityId })
    end)
    ...
    if success and error > 0 then
        ...
        for _, playerId in ipairs(Players) do
            ...
            if targetPlayer and Config.LawmenJobs[targetPlayer.PlayerData.job.name] then
                TriggerClientEvent('rsg-tips:client:removeActivityBlip', playerId, coords)
            end
        end
