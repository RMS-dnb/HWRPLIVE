...
RegisterNetEvent('rex-guarma:client:mainmenu', function(currentport)
    if currentport == 'vanhorn' then
        lib.registerContext({
            id = 'guarma_main_menu',
            title = 'Blackwater Port Menu',
            position = 'top-right',
            options = {
                {   
    ...
    elseif currentport == 'stdenis' then
        lib.registerContext({
            id = 'guarma_main_menu',
...
AddEventHandler('rex-guarma:client:dotravel', function(data)
    ...
    -- Require ticket for all travel
    if not hasItem then
        lib.notify({ title = locale('cl_lang_10'), duration = 5000, type = 'error' })
        return
    end

    -- Only enforce departure timing when going *TO GUARAMA*
    if data.destination == 'guarma' then
        if not ((minute >= 0 and minute <= 4) or (minute >= 30 and minute <= 34)) then
            lib.notify({ 
                title = "The boat departs at 00 and 30 minutes past each hour (5 min boarding window). Current time: "
    ...
    -- REMOVE TICKET (always remove regardless of direction)
    ...
    if data.destination == 'stdenis' then
        DoScreenFadeOut(500)
        Wait(1000)
        Citizen.InvokeNative(0x1E5B70E53DB661E5, 0, 0, 0, locale('cl_lang_16'), locale('cl_lang_8'), locale('cl_lang_9'))
        Wait(30000)
    ...
    if data.destination == 'vanhorn' then
        DoScreenFadeOut(500)
        Wait(1000)
        ...
        Citizen.InvokeNative(0xA657EC9DBC6CC900, -1868977180)
        Citizen.InvokeNative(0xE8770EE02AEE45C2, 0)
        Citizen.InvokeNative(0x203BEFFDBE12E96A, cache.ped, -718.86, -1278.43, 41.95 -1, 266.12268)
        ShutdownLoadingScreen()
        DoScreenFadeIn(1000)
        Wait(1000)
        SetCinematicModeActive(false)
