...
CreateThread(function()
    for _, v in ipairs(Config.WeaponsmithCrafting) do
        ...
        local option = {
            title = setheader,
            icon = itemimg,
            event = 'rex-weaponsmith:client:checkingredients',
...
CreateThread(function()
    for category, MenuData in pairs(CategoryMenus) do
        RegisterNetEvent('rex-weaponsmith:client:' .. category)
        AddEventHandler('rex-weaponsmith:client:' .. category, function()
...
RegisterNetEvent('rex-weaponsmith:client:craftingmenu', function()
    local Menu = {
        id = 'crafting_menu',
    ...
    for category, MenuData in pairs(CategoryMenus) do
        table.insert(Menu.options, {
            ...
            event = 'rex-weaponsmith:client:' .. category,
            arrow = true
        })
    end
...
-- -- craft item
-- ---------------------------------------------
-- RegisterNetEvent('rex-weaponsmith:client:checkingredients', function(data)
--     local hasItem = RSGCore.Functions.HasItem(data.bpc, 1)
--     if hasItem then
--         -- check crafting rep
--         RSGCore.Functions.TriggerCallback('rex-weaponsmith:server:checkxp', function(currentXP)
--             if currentXP >= data.craftingxp then
--                 -- check crafting items
--                 RSGCore.Functions.TriggerCallback('rex-weaponsmith:server:checkingredients', function(hasRequired)
...
--                         })
--                         TriggerServerEvent('rex-weaponsmith:server:finishcrafting', data)
...
---------------------------------------------
RegisterNetEvent('rex-weaponsmith:client:checkingredients', function(data)
    -- Create a table that includes all required ingredients
    local allRequiredItems = {}
    
    -- Add all ingredients to the required items (including the item name itself)
    for _, ingredient in pairs(data.ingredients) do
        table.insert(allRequiredItems, {item = ingredient.item, amount = ingredient.amount})
    end
    ...
    if Config.xpEnabled then
        ...
        RSGCore.Functions.TriggerCallback('rex-weaponsmith:server:checkxp', function(currentXP)
            if currentXP >= data.craftingxp then
                -- check ALL items (blueprint + ingredients)
                RSGCore.Functions.TriggerCallback('rex-weaponsmith:server:checkallitems', function(hasAllRequired)
                    if hasAllRequired == true then
                        LocalPlayer.state:set("inv_busy", true, true) -- lock inventory
                        lib.progressBar({
                            duration = tonumber(data.crafttime),
                            position = 'bottom',
                ...
                end, allRequiredItems)
            else
                lib.notify({ title = locale('cl_lang_52'), type = 'error', duration = 7000 })
            end
        end, 'crafting')
    else
        -- check ALL items (blueprint + ingredients) when XP is disabled
        RSGCore.Functions.TriggerCallback('rex-weaponsmith:server:checkallitems', function(hasAllRequired)
            if hasAllRequired == true then
                LocalPlayer.state:set("inv_busy", true, true) -- lock inventory
                lib.progressBar({
