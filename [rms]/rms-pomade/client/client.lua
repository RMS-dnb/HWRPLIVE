-- Event handler for pomade task
RegisterNetEvent("rsg-pomade:usepomade")
AddEventHandler("rsg-pomade:usepomade", function()
    local playerPed = PlayerPedId()
    if Citizen.InvokeNative(0xFB4891BD7578CDC1, playerPed, tonumber(0x9925C067)) then
    ...
    Wait(2500)
    Citizen.InvokeNative(0x66B957AAC2EAAEAB, playerPed, GetCurrentPedComponent(playerPed, "hair"), GetHashKey("POMADE"), 0, true, 1)
    Citizen.InvokeNative(0xCC8CA3E88256E58F, playerPed, false, true, true, true, false)
end)

-- Function to get the current ped component
function GetCurrentPedComponent(ped, category)
    ...
    for i = 0, componentsCount, 1 do
        ...
        if componentHash then
            ...
            if category ~= nil and GetHashKey(category) == componentCategoryHash then
...
-- Get number of components in Ped
...
-- Get shop ped component at index
...
-- Get shop ped component category
...
-- Initialization and settings for DataView
