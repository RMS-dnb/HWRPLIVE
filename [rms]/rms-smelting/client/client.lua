...
Citizen.CreateThread(function()
    for _,v in pairs(Config.SmeltingLocations) do
        exports['rsg-core']:createPrompt(v.prompt, v.coords, RSGCore.Shared.Keybinds[Config.Keybind], Lang:t('client.lang_1'), {
            type = 'client',
        ...
        if v.showblip == true then    
            ...
            SetBlipName(SmeltingBlip, Lang:t('client.lang_2'))
        end
    end
end)
...
RegisterNetEvent('rex-smelting:client:smeltitem', function(data)
    ...
    RSGCore.Functions.TriggerCallback('rex-smelting:server:checkingredients', function(hasRequired)
        if hasRequired == true then
            ...
            local smeltTime = tonumber(data.smelttime) or 5000 -- fallback 5 seconds
            ...
            lib.progressBar({
                ...
                label = Lang:t('client.lang_4') .. RSGCore.Shared.Items[data.receive].label,
            })

            TriggerServerEvent('rex-smelting:server:finishsmelting', data)
            LocalPlayer.state:set("inv_busy", false, true) -- unlock inventory
        else
            lib.notify({ title = Lang:t('client.lang_5'), type = 'inform', duration = 7000 })
        end
