...
CreateThread(function()
    for i = 1, #Config.Houses do
        ...
        exports.ox_target:addBoxZone({
            coords = house.menucoords,
            size = vec3(1.5, 1.5, 1.5),
            rotation = 0.0,
            debug = false,
            options = {
                {
                    name = 'house_menu_' .. house.houseid,
                    icon = 'fa-solid fa-house',
                    label = locale('cl_lang_15'),
                    onSelect = function()
                        TriggerEvent('rex-houses:client:housemenu', house.houseid)
                    end
                }
            }
        })
...
-- house storage
---------------------------------------
RegisterNetEvent("rex-houses:client:OpenHouseStorage", function(houseId)
    local citizenid = RSGCore.Functions.GetPlayerData().citizenid

    -- Sets the current stash for the client
    TriggerEvent("inventory:client:SetCurrentStash", houseId, citizenid)
    
    -- Calls server event to open the house storage inventory with predefined parameters
    TriggerServerEvent("rex-houses:server:OpenStorage", houseId)
end)
