...
RegisterNetEvent('rms-saloon:client:openInventory', function(saloonid)
   TriggerServerEvent('rex-saloon:server:Inventory', saloonid)
end)
---------------------------------------------
...
RegisterNetEvent('rex-saloon:client:rentsaloon', function(saloonid, name)
    ...
    -- Check if Config.PlayerSaloonLocations exists and is a table
    if not Config.PlayerSaloonLocations or type(Config.PlayerSaloonLocations) ~= 'table' then
        lib.notify({ 
            title = 'Configuration Error', 
            description = 'Saloon locations configuration is missing or invalid.',
            type = 'error', 
            duration = 7000 
        })
        return
    ...
    -- Find the required job for this specific saloon
    local requiredJob = nil
    for k, v in pairs(Config.PlayerSaloonLocations) do
        if v and v.saloonid == saloonid then
            requiredJob = v.jobaccess
            break
        end
    end
    
    -- Check if player has the required job for this saloon
    if not requiredJob or playerjob ~= requiredJob then
        lib.notify({ 
            title = 'Job Required', 
            description = 'You need to have the correct job to rent this saloon.',
            type = 'error', 
            duration = 7000 
        })
        return
