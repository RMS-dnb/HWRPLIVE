...
RegisterServerEvent('rex-saloon:server:Inventory', function(saloonid)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then return end
    local data = { label = 'Saloon Storage', maxweight = 2000000, slots = 64 }
    local stashName = 'slsto_'.. saloonid
    exports['rsg-inventory']:OpenInventory(src, stashName, data)
end)
