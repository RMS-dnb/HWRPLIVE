...
AddEventHandler('rex-goldpanning:client:startgoldpanning', function()
    ...
    -- Check if player has the goldpan item
    local hasItem = exports['rsg-inventory']:HasItem('goldpan')
    
    if not hasItem then
        lib.notify({ title = 'You don\'t have a gold pan!', type = 'error', duration = 7000 })
        return
    end
