...
RegisterNetEvent('rex-trapfishing:server:addbait', function(propid, newbait)
    ...
    -- check trap quality BEFORE anything else
    local result = MySQL.query.await('SELECT quality FROM rex_trapfishing WHERE propid = ?', {propid})
    if not result or not result[1] or result[1].quality <= 0 then
        TriggerClientEvent('ox_lib:notify', src, {title = locale('sv_lang_1'), description = 'Trap is broken', type = 'error', duration = 5000 })
        return
    end
