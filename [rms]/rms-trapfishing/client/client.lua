                ...
                options = {
                    {
                        ...
                        canInteract = function()
                            return not isBusy and not LocalPlayer.state.inv_busy
                        end,
                    },
                },
                distance = 3
...
-----------------------------------------------------------------
RegisterNetEvent('rex-trapfishing:client:emptytrap', function(data)
    if isBusy or LocalPlayer.state.inv_busy then 
        lib.notify({ title = locale('cl_lang_25'), type = 'error', duration = 7000 })
        return 
    end
    ...
    RSGCore.Functions.TriggerCallback('rex-trapfishing:server:getalltrapdata', function(result)
        ...
        -- Check if trap is empty
        if crayfish == 0 and lobster == 0 and crab == 0 and bluecrab == 0 then
            lib.notify({ title = "Trap is empty!", type = 'info', duration = 7000 })
            return
        end
        ...
        lib.registerContext({
            ...
            options = {
                ...
                },
                {
                    ...
                    args = { trapid = data.trapid },
                    arrow = true,
                    disabled = isBusy or LocalPlayer.state.inv_busy
                },
                {
                    title = locale('cl_lang_12'),
                    icon = 'fa-solid fa-rotate',
                    ...
                    args = { trapid = data.trapid },
                    arrow = true,
                    disabled = isBusy or LocalPlayer.state.inv_busy or (crayfish == 0 and lobster == 0 and crab == 0 and bluecrab == 0)
                },
                {
                    title = locale('cl_lang_13')..repaircost..locale('cl_lang_14'),
                    icon = 'fa-solid fa-screwdriver-wrench',
                    ...
                    args = { trapid = data.trapid },
                    arrow = true,
                    disabled = isBusy or LocalPlayer.state.inv_busy or quality >= 100
                },
                {
                    title = locale('cl_lang_15'),
                    icon = 'fa-solid fa-box-open',
...
RegisterNetEvent('rex-trapfishing:client:repairtrap', function(data)
    ...
    RSGCore.Functions.TriggerCallback('rex-trapfishing:server:getalltrapdata', function(result)
        ...
        local quality = result[1].quality
        local repaircost = (100 - quality) * Config.RepairCost

        if quality >= 100 then
            lib.notify({ title = "Trap is already in perfect condition!", type = 'info', duration = 7000 })
            return
        end
...
AddEventHandler('rex-trapfishing:client:addbait', function(data)
    ...
    RSGCore.Functions.TriggerCallback('rex-trapfishing:server:getalltrapdata', function(result)
        ...
        -- check if trap is broken FIRST
        if quality <= 0 then
            lib.notify({ title = locale('cl_lang_1'), description = 'Trap is broken', type = 'error', duration = 7000 })
            return
        end
