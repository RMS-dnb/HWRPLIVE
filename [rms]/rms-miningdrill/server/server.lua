...
local function getWeightedItem(outputTable)
    local totalWeight = 0
    for _, data in ipairs(outputTable) do
        totalWeight = totalWeight + data.percentage
    end

    if totalWeight <= 0 then return nil end

    local rand = math.random(1, totalWeight * 10) / 10
    local cumulative = 0
    ...
    -- fallback
    return outputTable[#outputTable].item
end
---------------------------------
...
RegisterNetEvent('rex-mining:server:finishdrilling', function(item)
    ...
    if item == 'rock' then
        local randomItem = getWeightedItem(Config.RockOutputs)
        Player.Functions.RemoveItem(item, 1)
        Player.Functions.AddItem(randomItem, 1)
        TriggerClientEvent('rNotify:ShowAdvancedRightNotification', src, "1 x "..RSGCore.Shared.Items[randomItem].label, "generic_textures" , "tick" , "COLOR_PURE_WHITE", 1000)
        ...
        if gemchance <= Config.GemChance then
            ...
            TriggerClientEvent('rNotify:ShowAdvancedRightNotification', src, "1 x "..RSGCore.Shared.Items[randomGem].label, "generic_textures" , "tick" , "COLOR_PURE_WHITE", 1000)
        end
    elseif item == 'saltrock' then
        local randomItem = getWeightedItem(Config.SaltRockOutputs)
        Player.Functions.RemoveItem(item, 1)
        Player.Functions.AddItem(randomItem, 1)
        TriggerClientEvent('rNotify:ShowAdvancedRightNotification', src, "1 x "..RSGCore.Shared.Items[randomItem].label, "generic_textures" , "tick" , "COLOR_PURE_WHITE", 1000)
    end
...
-- Set up a basic cooldown timer instead of cron job
local miningCooldown = 15000 -- 15 seconds in milliseconds

-- Initialize the repeating timer function
Citizen.CreateThread(function()
    while true do
        -- Run the mining update logic
        local result = MySQL.query.await('SELECT * FROM rex_mining')
        if result then
            for i = 1, #result do
