Config = {}

---------------------------------
-- settings
---------------------------------
Config.EnableTarget     = true
Config.Keybind          = 'J'
Config.Img              = "rsg-inventory/html/images/"
Config.Money            = 'cash' -- 'cash', 'bank' or 'bloodmoney'
Config.ServerNotify     = true
Config.LicenseRequired  = false
Config.EnableRentSystem = true

---------------------------------
-- rent settings
---------------------------------
Config.MaxBlacksmiths    = 1
Config.RentStartup        = 100
Config.RentPerHour        = 1
Config.BlacksmithCronJob = '0 * * * *' -- cronjob runs every hour (0 * * * *)
Config.MaxRent            = 750

---------------------------------
-- storage settings
---------------------------------
Config.PlayerShareMaxWeight = 4000000
Config.PlayerShareMaxSlots  = 4
Config.PlayerShareName      = 'Player Share Storage'
Config.CraftingMaxWeight    = 4000000
Config.CraftingMaxSlots     = 50
Config.CraftingName         = 'Crafting Storage'
Config.StockMaxWeight       = 4000000
Config.StockMaxSlots        = 100
Config.StockName            = 'Stock Storage'

---------------------------------
-- npc settings
---------------------------------
Config.DistanceSpawn = 20.0
Config.FadeIn = true

---------------------------------
-- blacksmith locations
---------------------------------
Config.BlacksmithLocations = {
    { 
        name = 'Valentine Blacksmith',
        blacksmithid = 'valblacksmith',
        coords = vector3(-369.29, 795.93, 116.19),
        npcmodel = `mp_u_M_M_lom_rhd_smithassistant_01`,
        npccoords = vector4(-369.29, 795.93, 116.19, 320.60),
        jobaccess = 'valblacksmith',
        blipname = 'Valentine Blacksmith',
        blipsprite = 'blip_shop_blacksmith',
        blipscale = 0.2,
        showblip = true
    },
    { 
        name = 'Blackwater Blacksmith',
        blacksmithid = 'blkblacksmith',
        coords = vector3(-869.60, -1394.87, 43.53),
        npcmodel = `mp_u_M_M_lom_rhd_smithassistant_01`,
        npccoords = vector4(-869.60, -1394.87, 43.53, 138.74),
        jobaccess = 'blkblacksmith',
        blipname = 'Blackwater Blacksmith',
        blipsprite = 'blip_shop_blacksmith',
        blipscale = 0.2,
        showblip = true
    },
    { 
        name = 'Van Horn Blacksmith',
        blacksmithid = 'vanblacksmith',
        coords = vector3(2953.1, 557.94, 49.21),
        npcmodel = `mp_u_M_M_lom_rhd_smithassistant_01`,
        npccoords = vector3(2953.1, 557.94, 49.21),
        jobaccess = 'vanblacksmith',
        blipname = 'Van Horn Blacksmith',
        blipsprite = 'blip_shop_blacksmith',
        blipscale = 0.2,
        showblip = true
    },
    { 
        name = 'St. Denis Blacksmith',
        blacksmithid = 'stdblacksmith',
        coords = vector3(2514.80, -1456.68, 46.31),
        npcmodel = `mp_u_M_M_lom_rhd_smithassistant_01`,
        npccoords = vector4(2514.80, -1456.68, 46.31, 101.11),
        jobaccess = 'stdblacksmith',
        blipname = 'St. Denis Blacksmith',
        blipsprite = 'blip_shop_blacksmith',
        blipscale = 0.2,
        showblip = true
    },
    { 
        name = 'Strawberry Blacksmith',
        blacksmithid = 'strblacksmith',
        coords = vector3(-1819.69, -568.37, 155.99),
        npcmodel = `mp_u_M_M_lom_rhd_smithassistant_01`,
        npccoords = vector4(-1819.69, -568.37, 155.99, 208.65),
        jobaccess = 'strblacksmith',
        blipname = 'Strawberry Blacksmith',
        blipsprite = 'blip_shop_blacksmith',
        blipscale = 0.2,
        showblip = true
    },
    { 
        name = 'MacFarlane Blacksmith',
        blacksmithid = 'macblacksmith',
        coords = vector3(-2395.84, -2378.86, 61.19),
        npcmodel = `mp_u_M_M_lom_rhd_smithassistant_01`,
        npccoords = vector4(-2395.84, -2378.86, 61.19, 110.73),
        jobaccess = 'macblacksmith',
        blipname = 'MacFarlane Blacksmith',
        blipsprite = 'blip_shop_blacksmith',
        blipscale = 0.2,
        showblip = true
    },
    { 
        name = 'Colter Blacksmith',
        blacksmithid = 'spiblacksmith',
        coords = vector3(-1348.27, 2404.61, 307.06),
        npcmodel = `mp_u_M_M_lom_rhd_smithassistant_01`,
        npccoords = vector4(-1348.27, 2404.61, 307.06, 273.74),
        jobaccess = 'spiblacksmith',
        blipname = 'Colter Blacksmith',
        blipsprite = 'blip_shop_blacksmith',
        blipscale = 0.2,
        showblip = true
    },
    { 
        name = 'Tumbleweed Blacksmith',
        blacksmithid = 'tumblacksmith',
        coords = vector3(-5516.69, -3040.95, -2.39),
        npcmodel = `mp_u_M_M_lom_rhd_smithassistant_01`,
        npccoords = vector4(-5516.69, -3040.95, -2.39, 209.54),
        jobaccess = 'tumblacksmith',
        blipname = 'Tumbleweed Blacksmith',
        blipsprite = 'blip_shop_blacksmith',
        blipscale = 0.2,
        showblip = true
    },
    { 
        name = 'Armadillo Blacksmith',
        blacksmithid = 'armablacksmith',
        coords = vector3(-3681.31, -2565.66, -13.56),
        npcmodel = `mp_u_M_M_lom_rhd_smithassistant_01`,
        npccoords = vector4(-3679.06, -2564.37, -13.56, 187.61),
        jobaccess = 'armablacksmith',
        blipname = 'Armadillo Blacksmith',
        blipsprite = 'blip_shop_blacksmith',
        blipscale = 0.2,
        showblip = true
    },
    { 
        name = 'Emerald Ranch Blacksmith',
        blacksmithid = 'emerblacksmith',
        coords = vector3(1442.63, 266.45, 90.35),
        npcmodel = `mp_u_M_M_lom_rhd_smithassistant_01`,
        npccoords = vector4(1433.19, 267.32, 90.34, 43.68),
        jobaccess = 'emerblacksmith',
        blipname = 'Emerald Ranch Blacksmith',
        blipsprite = 'blip_shop_blacksmith',
        blipscale = 0.2,
        showblip = true
    },
    { 
        name = 'Rhodes Blacksmith',
        blacksmithid = 'rhoblacksmith',
        coords = vector3(1355.57, -1248.36, 79.99),
        npcmodel = mp_u_M_M_lom_rhd_smithassistant_01,
        npccoords = vector4(1354.05, -1249.31, 80.03, 7),
        jobaccess = 'rhoblacksmith',
        blipname = 'Rhodes Blacksmith',
        blipsprite = 'blip_shop_blacksmith',
        blipscale = 0.2,
        showblip = true
    },
}

---------------------------------
-- blacksmith crafting
---------------------------------
Config.BlacksmithCrafting = {

    {
        category = 'Tools',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_pickaxe',
        ingredients = { 
            [1] = { item = 'coal',    amount = 2 },
            [2] = { item = 'steel_bar',   amount = 2 },
            [3] = { item = 'wood',        amount = 2 },
        },
        receive = 'pickaxe',
        giveamount = 1
    },
    {
        category = 'Tools',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_axe',
        ingredients = { 
            [1] = { item = 'coal',    amount = 2 },
            [2] = { item = 'steel_bar',   amount = 1 },
            [3] = { item = 'wood',        amount = 2 },
        },
        receive = 'axe',
        giveamount = 1
    },
    {
        category = 'Tools',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_shovel',
        ingredients = { 
            [1] = { item = 'coal',    amount = 2 },
            [2] = { item = 'steel_bar',   amount = 1 },
            [3] = { item = 'wood',        amount = 2 },
        },
        receive = 'shovel',
        giveamount = 1
    },
    {
        category = 'Tools',
        crafttime = 30000,
        ingredients = { 
            [1] = { item = 'coal',  amount = 5 },
            [2] = { item = 'iron_bar', amount = 2 },
            [3] = { item = 'silver_bar',      amount = 1 },
            [4] = { item = 'animalfat',      amount = 1 },
        },
        receive = 'horse_lantern',
        giveamount = 1
    },
    {
        category = 'Tools',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_weapon_melee_lantern',
        ingredients = { 
            [1] = { item = 'coal',  amount = 5 },
            [2] = { item = 'iron_bar', amount = 2 },
            [3] = { item = 'cotton',      amount = 1 },
            [4] = { item = 'animalfat',      amount = 2 },
        },
        receive = 'weapon_melee_lantern',
        giveamount = 1
    },
    {
        category = 'Tools',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_weapon_melee_davy_lantern',
        ingredients = { 
            [1] = { item = 'coal',  amount = 5 },
            [2] = { item = 'iron_bar', amount = 1 },
            [3] = { item = 'copper_bar',      amount = 2 },
            [4] = { item = 'zinc_ore',      amount = 1 },
        },
        receive = 'weapon_melee_davy_lantern',
        giveamount = 1
    },
    {
        category = 'Tools',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_weapon_melee_lantern_halloween',
        ingredients = { 
            [1] = { item = 'coal',  amount = 5 },
            [2] = { item = 'steel_bar', amount = 2 },
            [3] = { item = 'cotton',      amount = 2 },
            [4] = { item = 'animalfat',      amount = 2 },
        },
        receive = 'weapon_melee_lantern_halloween',
        giveamount = 1
    },
    {
        category = 'Tools',
        crafttime = 30000,
        craftingxp = 0,
        ingredients = { 
            [1] = { item = 'coal',  amount = 10 },
            [2] = { item = 'steel_bar', amount = 5 },
            [3] = { item = 'brass_bar', amount = 2 },
        },
        receive = 'furnace',
        giveamount = 1
    },
    {
        category = 'Hatchets',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_weapon_melee_hatchet',
        ingredients = { 
            [1] = { item = 'coal',  amount = 3 },
            [2] = { item = 'steel_bar', amount = 1 },
            [3] = { item = 'wood',      amount = 2 },
        },
        receive = 'weapon_melee_hatchet',
        giveamount = 1
    },
    
    {
        category = 'Hatchets',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_weapon_melee_hatchet_hunter',
        ingredients = { 
            [1] = { item = 'coal',  amount = 3 },
            [2] = { item = 'steel_bar', amount = 1 },
            [3] = { item = 'wood',      amount = 2 },
        },
        receive = 'weapon_melee_hatchet_hunter',
        giveamount = 1
    },
    {
        category = 'Hatchets',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_weapon_melee_cleaver',
        ingredients = { 
            [1] = { item = 'coal',  amount = 3 },
            [2] = { item = 'steel_bar', amount = 1 },
            [3] = { item = 'wood',      amount = 2 },
        },
        receive = 'weapon_melee_cleaver',
        giveamount = 1
    },
    {
        category = 'Knives',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_weapon_thrown_throwing_knives',
        ingredients = { 
            [1] = { item = 'coal',  amount = 4 },
            [2] = { item = 'steel_bar', amount = 2 },
            [3] = { item = 'wood',      amount = 4 },
        },
        receive = 'weapon_thrown_throwing_knives',
        giveamount = 4
    },
    
  --  {
  --      category = 'Knives',
  --      crafttime = 30000,
  --      craftingxp = 0,
  --      bpc = 'bpc_weapon_melee_knife_jawbone',
  --      ingredients = { 
  --          [1] = { item = 'coal',  amount = 1 },
  --          [2] = { item = 'steel_bar', amount = 1 },
  --          [3] = { item = 'wood',      amount = 2 },
  --      },
  --      receive = 'weapon_melee_knife_jawbone',
  --      giveamount = 1
  --  },
    -- {
    --     category = 'Knives',
    --     crafttime = 30000,
    --     craftingxp = 0,
    --     bpc = 'bpc_weapon_melee_knife_trader',
    --     ingredients = { 
    --         [1] = { item = 'coal',  amount = 1 },
    --         [2] = { item = 'steel_bar', amount = 1 },
    --         [3] = { item = 'wood',      amount = 2 },
    --     },
    --     receive = 'weapon_melee_knife_trader',
    --     giveamount = 1
    -- },
    {
        category = 'Knives',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_weapon_melee_knife_horror',
        ingredients = { 
            [1] = { item = 'coal',  amount = 3 },
            [2] = { item = 'steel_bar', amount = 2 },
            [3] = { item = 'small_pelt',      amount = 1 },
        },
        receive = 'weapon_melee_knife_horror',
        giveamount = 1
    },
    -- {
    --     category = 'Knives',
    --     crafttime = 30000,
    --     craftingxp = 0,
    --     bpc = 'bpc_weapon_melee_knife_rustic',
    --     ingredients = { 
    --         [1] = { item = 'coal',  amount = 1 },
    --         [2] = { item = 'steel_bar', amount = 1 },
    --         [3] = { item = 'wood',      amount = 2 },
    --     },
    --     receive = 'weapon_melee_knife_rustic',
    --     giveamount = 1
    -- },
    {
        category = 'Machete',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_weapon_melee_machete',
        ingredients = { 
            [1] = { item = 'coal',  amount = 4 },
            [2] = { item = 'steel_bar', amount = 2 },
            [3] = { item = 'wood',      amount = 2 },
        },
        receive = 'weapon_melee_machete',
        giveamount = 1
    },
    {
        category = 'Machete',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_weapon_melee_machete_horror',
        ingredients = { 
            [1] = { item = 'coal',  amount = 4 },
            [2] = { item = 'steel_bar', amount = 2 },
            [3] = { item = 'silver_bar', amount = 1 },
            [4] = { item = 'ruby', amount = 2 },
            [5] = { item = 'wood',      amount = 2 },
        },
        receive = 'weapon_melee_machete_horror',
        giveamount = 1
    },
    {
        category = 'Machete',
        crafttime = 30000,
        craftingxp = 0,
      --bpc = 'bpc_weapon_melee_machete_collector',
        ingredients = { 
            [1] = { item = 'coal',  amount = 4 },
            [2] = { item = 'steel_bar', amount = 2 },
            [3] = { item = 'gold_bar', amount = 1 },
            [4] = { item = 'sapphire', amount = 2 },
            [5] = { item = 'wood',      amount = 2 },
        },
        receive = 'weapon_melee_machete_collector',
        giveamount = 1
    },
    {
        category = 'Tools',
        crafttime = 30000,
        craftingxp = 0,
        ingredients = { 
            [1] = { item = 'coal',  amount = 3 },
            [2] = { item = 'steel_bar', amount = 1 },
            [3] = { item = 'small_pelt',      amount = 1 },
            [4] = { item = 'wood',      amount = 2 },
        },
        receive = 'weapon_melee_hammer',
        giveamount = 1
    },
--    {
--        category = 'Tools',
--        crafttime = 30000,
--        craftingxp = 0,
--        bpc = 'bpc_weapon_melee_torch',
--        ingredients = { 
--           [1] = { item = 'coal',  amount = 1 },
--            [2] = { item = 'steel_bar', amount = 1 },
--            [3] = { item = 'wood',      amount = 2 },
--        },
--        receive = 'weapon_melee_torch',
--        giveamount = 1
--    },
    -- {
    --     category = 'Tools',
    --     crafttime = 30000,
        
    --     ingredients = { 
    --         [1] = { item = 'coal',  amount = 1 },
    --         [2] = { item = 'steel_bar', amount = 1 },
    --         [3] = { item = 'wood',      amount = 2 },
    --     },
    --     receive = 'cage',
    --     giveamount = 1
    -- },
    {
        category = 'Tools',
        crafttime = 30000,
        
        ingredients = { 
            [1] = { item = 'coal',  amount = 10 },
            [2] = { item = 'steel_bar', amount = 5 },
            [3] = { item = 'iron_bar',      amount = 2 },
            [4] = { item = 'wood',      amount = 8 },
        },
        receive = 'stashchest',
        giveamount = 1
    },
    {
        category = 'Tools',
        crafttime = 30000,
        
        ingredients = { 
            [1] = { item = 'coal',  amount = 10 },
            [2] = { item = 'steel_bar', amount = 5 },
            [3] = { item = 'iron_bar',      amount = 2 },
        },
        receive = 'moneybox',
        giveamount = 1
    },
    {
        category = 'Items',
        crafttime = 30000,
        
        ingredients = { 
            [1] = { item = 'coal',  amount = 3 },
            [2] = { item = 'brass_bar', amount = 1 },
            [3] = { item = 'cotton',      amount = 1 },
            [3] = { item = 'animalfat',      amount = 2 },
        },
        receive = 'zippo',
        giveamount = 2
    },
    {
        category = 'Tools',
        crafttime = 30000,
        -- craftingxp = 0,
        -- bpc = 'bpc_axe',
        ingredients = { 
            [1] = { item = 'coal',  amount = 3 },
            [2] = { item = 'iron_bar', amount = 2 },
        },
        receive = 'goldpan',
        giveamount = 1
    },
    -- {
    --     category = 'Gold Panning',
    --     crafttime = 30000,
    --     -- craftingxp = 0,
    --     -- bpc = 'bpc_axe',
    --     ingredients = { 
    --         [1] = { item = 'coal',  amount = 1 },
    --         [2] = { item = 'steel_bar', amount = 1 },
    --         [3] = { item = 'wood',      amount = 2 },
    --     },
    --     receive = 'gr_bucket',
    --     giveamount = 1
    -- },
    -- {
    --     category = 'Gold Panning',
    --     crafttime = 30000,
    --     -- craftingxp = 0,
    --     -- bpc = 'bpc_axe',
    --     ingredients = { 
    --         [1] = { item = 'coal',  amount = 1 },
    --         [2] = { item = 'steel_bar', amount = 1 },
    --         [3] = { item = 'wood',      amount = 2 },
    --     },
    --     receive = 'gr_shovel',
    --     giveamount = 1
    -- },
    -- {
    --     category = 'Repairs',
    --     crafttime = 30000,
    --     ingredients = { 
    --         [1] = { item = 'brokenaxe',  amount = 1 },
    --         [2] = { item = 'steel_bar', amount = 1 },
    --         [3] = { item = 'wood',      amount = 2 },
    --     },
    --     receive = 'axe',
    --     giveamount = 1
    -- },
    -- {
    --     category = 'Repairs',
    --     crafttime = 30000,
    --     ingredients = { 
    --         [1] = { item = 'brokenpickaxe',  amount = 1 },
    --         [2] = { item = 'steel_bar', amount = 1 },
    --         [3] = { item = 'wood',      amount = 2 },
    --     },
    --     receive = 'pickaxe',
    --     giveamount = 1
    -- },
    

}





