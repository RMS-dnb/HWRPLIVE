...
RegisterNetEvent('rex-blacksmith:client:rentblacksmith', function(blacksmithid, name)
    ...
    -- Find the required job for this specific blacksmith
    local requiredJob = nil
    for k, v in pairs(Config.BlacksmithLocations) do
        if v.blacksmithid == blacksmithid then
            requiredJob = v.jobaccess
            break
        end
    end
    
    -- Check if player has the required job for this blacksmith
    if not requiredJob or playerjob ~= requiredJob then
        lib.notify({ 
            title = 'Job Required', 
            description = 'You need to have the correct job to rent this blacksmith shop.',
            type = 'error', 
            duration = 7000 
        })
        return
