...
function CreateBoatPreviewCamera(shopId)
    ...
    SetCamCoord(BoatCam, previewX - 40.0, previewY - 40.0, previewZ + 25.0)
    SetCamActive(BoatCam, true)
    PointCamAtCoord(BoatCam, previewX, previewY, previewZ)
    SetCamFov(BoatCam, 45.0)
...
function OpenBoatInventory(invId, boatModel)
    -- Simple stash opening - no need to check config
    TriggerServerEvent('sm-boats:server:OpenBoatStash', invId)
end
...
function SpawnBoat()
    ...
    SetEntityInvincible(MyBoat, false)
    SetVehicleDoorsLocked(MyBoat, 0)
    DoScreenFadeOut(500)
    Wait(500)
    ...
    exports.ox_target:addLocalEntity(MyBoat, {
        ...
        {
            name = 'boat_anchor_' .. OwnedData.id,
            icon = 'fas fa-anchor',
            label = 'Toggle Anchor',
            onSelect = function()
                ToggleAnchor()
        ...
        {
            name = 'boat_fishing_' .. OwnedData.id,
            icon = 'fas fa-fish',
            label = 'Deploy Fishing Net',
            canInteract = function()
                return isAnchored
            end,
            onSelect = function()
        ...
        {
            name = 'boat_drive_' .. OwnedData.id,
            icon = 'fas fa-ship',
            label = 'Enter Driver Seat',
            canInteract = function()
                local player = PlayerPedId()
                return not IsPedInVehicle(player, MyBoat, false)
            end,
            onSelect = function()
                local player = PlayerPedId()
                SetPedIntoVehicle(player, MyBoat, -1)
...
-- Add this function to toggle anchor
function ToggleAnchor()
    ...
    if isAnchored then
        ...
        jo.notif.right("Anchor dropped", 'inventory_items', 'generic_exotic_order', 'color_white', 3000)
    else
        FreezeEntityPosition(MyBoat, false)
        SetBoatAnchor(MyBoat, false)
        jo.notif.right("Anchor raised", 'inventory_items', 'generic_exotic_order', 'color_white', 3000)
    end
end
...
AddEventHandler('baseevents:vehicleLeft', function(vehicle, seat)
    local ped = PlayerPedId()
    if seat == -1 then -- -1 = driver
        myBoat = vehicle
    end
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if myBoat and DoesEntityExist(myBoat) then
            local boatCoords = GetEntityCoords(myBoat)
            local distance = #(GetEntityCoords(ped) - boatCoords)

            if distance < 3.0 then
                -- Display help text
                DrawText3D(boatCoords.x, boatCoords.y, boatCoords.z + 1.0, "[E] Enter Ship")
                
                if IsControlJustPressed(0, 38) then -- E key
                    TaskEnterVehicle(ped, myBoat, -1, -1, 1.0, 1, 0)
                end
            end
        end
    end
...
-- function StartFishingNet()
--     if not MyBoat then return end
--     if not isAnchored then
--         jo.notif.right("You need to anchor the boat first!", 'inventory_items', 'generic_exotic_order', 'COLOR_RED', 3000)
--         return
--     end
    
--     -- Lowering net progress bar
--     if lib.progressBar({
--         duration = math.random(5000, 10000),
--         label = 'Lowering Fishing Net...',
--         useWhileDead = false,
--         canCancel = true,
--         disable = {
--             car = true,
...
--         }
--     }) then
--         -- Waiting for fish progress bar
--         if lib.progressBar({
--             duration = math.random(5000, 10000),
--             label = 'Waiting for Fish...',
--             useWhileDead = false,
--             canCancel = true,
--             disable = {
--                 car = true,
...
--             if success then
--                 -- Pulling up net with fish
--                 if lib.progressBar({
--                     duration = math.random(5000, 10000),
--                     label = 'Pulling Up Fishing Net...',
--                     useWhileDead = false,
--                     canCancel = false,
--                     disable = {
--                         car = true,
...
--                 }) then
--                     -- Give rewards
--                     TriggerServerEvent('sm-boats:GetFishingRewards')
--                 end
--             else
--                 -- Failed skill check
--                 if lib.progressBar({
--                     duration = math.random(3000, 5000),
--                     label = 'Pulling Up Empty Net...',
--                     useWhileDead = false,
--                     canCancel = false,
--                     disable = {
--                         car = true,
...
--                     }
--                 }) then
--                     jo.notif.right("The fish got away!", 'inventory_items', 'generic_exotic_order', 'COLOR_RED', 3000)
--                 end
--             end
--         else
--             jo.notif.right("You cancelled fishing!", 'inventory_items', 'generic_exotic_order', 'COLOR_RED', 3000)
--         end
--     else
--         jo.notif.right("You cancelled fishing!", 'inventory_items', 'generic_exotic_order', 'COLOR_RED', 3000)
--     end
...
function StartFishingNet()
    ...
    -- Ask server to check for fishingnet & raw_fish
    RSGCore.Functions.TriggerCallback('sm-boats:CheckFishingItems', function(hasItems, usesLeft)
    if not hasItems then
        jo.notif.right("You need a Fishing Net and at least 1 Raw Fish!", 'inventory_items', 'generic_exotic_order', 'COLOR_RED', 3000)
        return
    end

    -- ✅ Consume bait before starting
    TriggerServerEvent('sm-boats:ConsumeFishBait')

        -- Proceed with your existing fishing logic
        if lib.progressBar({
            duration = math.random(5000, 10000),
            label = 'Lowering Fishing Net...',
            useWhileDead = false,
            ...
            if lib.progressBar({
                ...
                if success then
                    if lib.progressBar({
                        ...
                        -- Reduce durability after success
                        TriggerServerEvent('sm-boats:ReduceNetDurability')
                    end
                else
                    if lib.progressBar({
                        duration = math.random(3000, 5000),
                        ...
                        jo.notif.right("The fish got away!", 'inventory_items', 'generic_exotic_order', 'COLOR_RED', 3000)
                        TriggerServerEvent('sm-boats:ReduceNetDurability') -- Still wear and tear
                    end
                end
            else
                jo.notif.right("You cancelled fishing!", 'inventory_items', 'generic_exotic_order', 'COLOR_RED', 3000)
...
-- Add this event to receive the fishing success notification
RegisterNetEvent('sm-boats:client:FishingSuccess')
AddEventHandler('sm-boats:client:FishingSuccess', function(count, fishList)
    jo.notif.right("You caught " .. count .. " fish: " .. fishList, 'inventory_items', 'consumable_fish', 'color_white', 7000)
end)