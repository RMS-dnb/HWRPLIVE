...
RegisterNetEvent('rsg-inventory:server:useItem', function(item)
    ...
    if itemData.type == 'weapon' then
        local result = MySQL.Sync.fetchAll('SELECT * FROM player_weapons WHERE serial = @serial and citizenid = @citizenid',{ serial = itemData.info.serial, citizenid = Player.PlayerData.citizenid })
        if result[1] == nil then
            local params = { serial = itemData.info.serial, citizenid = Player.PlayerData.citizenid }
...
RegisterNetEvent('rsg-inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
    ...
    if blockedItems[fromItem.name] and toInventory ~= 'player' then
        return
    end
    ...
    elseif not toItem and toAmount < fromAmount then
        ...
        if toItem then
            ...
            if Inventory.RemoveItem(fromId, fromItem.name, fromItemAmount, fromSlot, 'swapped item', isMove)
                and Inventory.RemoveItem(toId, toItem.name, toItemAmount, toSlot, 'swapped item', isMove) then
