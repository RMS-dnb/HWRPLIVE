...
Inventory.SetInventory = function(source, items)
    ...
    if not Player.Offline then
        ...
        TriggerEvent('rsg-log:server:CreateLog', 'playerinventoryset', 'SetInventory', 'blue', logMessage)
    end
end
...
Inventory.ClearInventory = function(source, filterItems)
    ...
    if not player.Offline then
        ...
        TriggerEvent('rsg-log:server:CreateLog', 'playerinventoryclear', 'ClearInventory', 'red', logMessage)
...
Inventory.AddItem = function(identifier, item, amount, slot, info, reason)
    ...
    local total = totalWeight or 0
    local maxWeight = inventoryWeight or 0

    if total + (weight * amt) > maxWeight then
        print('AddItem: Not enough weight available')
        return false
    end
    ...
    if not updated then
        ...
        if itemInfo.type == 'weapon' then
            if not inventory[slot].info.serial then
                inventory[slot].info.serial = tostring(
