const InventoryContainer = Vue.createApp({
    ...
    // ADD WEIGHT VALIDATION FOR BOTH DIRECTIONS
    if (targetInventoryType === "other") {
        ...
        if (totalWeightAfterTransfer > this.otherInventoryMaxWeight) {
            // stop transfer + warn
            fetch(`https://${GetParentResourceName()}/inventory:notify`, {
                method: "POST",
                headers: { "Content-Type": "application/json; charset=UTF-8" },
                body: JSON.stringify({
                    message: "Inventory is full! Cannot add more items.",
                    type: "error"
                })
            });
            this.busy = false;
    ...
    // Remove the old weight check that was only for other inventory
    // const totalWeightAfterTransfer = this.otherInventoryWeight + sourceItem.weight * amountToTransfer;
    // if (totalWeightAfterTransfer > this.otherInventoryMaxWeight) {
    //     this.inventoryError(item.slot);
    //     this.busy = false;
