...
RegisterCommand('opencreator', function(source, args, rawCommand)
    ...
    if Player and Player.PlayerData.job.name == "admin" then
        TriggerClientEvent('rsg-appearance:client:OpenCreator', targetId, {cid = TargetPlayer.PlayerData.citizenid}, false)
        TriggerClientEvent('ox_lib:notify', src, { 
            title = 'Success', 
            description = 'Opened appearance creator for Player ID: ' .. targetId, 
            type = 'success' 
        })
    else
        TriggerClientEvent('ox_lib:notify', src, { 
            title = 'Error', 
            description = 'You do not have admin job permission to use this command!', 
            type = 'error' 
        })
    end
end, false)
