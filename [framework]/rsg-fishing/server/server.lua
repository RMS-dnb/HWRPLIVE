...
AddEventHandler('rsg-fishing:FishToInventory', function(fishModel, weight)
    ...
    local fish = fishEntity[fishModel]
    local fish_name = fishNames[fishModel]
    if not fish or not fish_name then return end

    local fish_weight = string.format('%.2f', (weight * 54.25))

    -- Add fish to inventory with weight metadata
    Player.Functions.AddItem(fish, 1, nil, { weight = fish_weight })

    -- Show inventory item box
    TriggerClientEvent('rsg-inventory:client:ItemBox', src, RSGCore.Shared.Items[fish], 'add', 1)

    -- Send success notification
    TriggerClientEvent('ox_lib:notify', src, {
        title = locale('sv_you_got_fish_name') .. ' ' .. fish_name,
        type = 'success',
        duration = 5000
    })

    -- Safely get player name for logging
    local first = Player.PlayerData.firstname or (Player.PlayerData.charinfo and Player.PlayerData.charinfo.firstname) or 'Unknown'
    local last = Player.PlayerData.lastname or (Player.PlayerData.charinfo and Player.PlayerData.charinfo.lastname) or ''
    local fullName = first .. ' ' .. last

    -- Log to Discord
    TriggerEvent('rsg-log:server:CreateLog', 'fishing', locale('sv_discord_b'), 'green',
        fullName .. ' ' .. locale('sv_discord_c') .. ' ' .. fish_weight .. 'KG ' .. fish_name
    )
end)
