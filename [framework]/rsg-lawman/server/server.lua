...
RSGCore.Commands.Add('lawbadge', locale('sv_lawbadge'), {}, false, function(source, args)
    ...
    if onduty and jobname == 'lso' or jobname == 'nhso' or jobname == 'naso' or jobname == 'usms' or jobname == 'doj' then
...
RegisterNetEvent('rsg-lawman:server:lawmanAlert', function(text, coords)
    ...
    for _, v in pairs(players) do
        -- Only send to law enforcement who are ON DUTY
        if v and v.PlayerData and v.PlayerData.job and
           v.PlayerData.job.type == 'leo' and 
           v.PlayerData.job.onduty == true then -- Added onduty check
...
--             description = 'Only law enforcement can jail players', 
--             type = 'error', 
--             duration = 5000 
--         })
--     end
...
-- Add command to trigger the menu
-- RSGCore.Commands.Add('jailer', 'Open Jail Management Menu', {}, true, function(source)
--     TriggerEvent('rsg-lawman:client:JailerMenu')
-- end)
...
--     if CuffedPlayer and Player then
--         if Player.Functions.GetItemByName('handcuffs') then
--             TriggerClientEvent('rsg-lawman:client:getcuffed', CuffedPlayer.PlayerData.source, Player.PlayerData.source, isSoftcuff)
--         end
--     end
...
lib.callback.register('btc-fines:server:getAllUnpaidFines', function(source)
    local results = MySQL.query.await('SELECT * FROM btc_fines WHERE paid = 0')
...
lib.callback.register('rsg-lawman:server:CheckJobType', function(source)
    local Player = RSGCore.Functions.GetPlayer(source)
    if not Player then return false end
    local playerType = Player.PlayerData.job.type

    if playerType == 'leo' then
        return true
    end
...
lib.callback.register('rsg-lawman:server:CheckJob', function(source)
    local Player = RSGCore.Functions.GetPlayer(source)
...
RegisterServerEvent('rsg-lawman:server:storageNames', function(jobname)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then return end
    
    local data = { label = locale('sv_storage') .. ' ' .. jobname, maxweight = Config.StorageMaxWeight, slots = Config.StorageMaxSlots }
    local stashName = 'lawstorage_' .. jobname
