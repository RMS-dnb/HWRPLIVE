...
CreateThread(function()
    for _,v in pairs(Config.BankLocations) do
        if Config.UseTarget then
            ...
            --     id = v.name, -- needed for removing interactions
            --     options = {
            --          {
            --             label = 'Bank Teller',
...
AddEventHandler('RSGCore:Client:OnPlayerLoaded', function()
   local isLaw = lib.callback.await('rsg-banking:server:checkLawmen', false)
    if isLaw then
            ...
            onSelect = function(data)
                ...
                if isLaw then
                    ...
                    if IsEntityAPed(targetEntity) and IsPedAPlayer(targetEntity) then
                        ...
                        if targetServerId and targetServerId > 0 then
                            CreateFineForPlayer(targetServerId)
                        end
                    end
                end
            end,
...
RegisterCommand('debug', function()
    local src = source
    CreateFineForPlayer(3)
end, false)

function CreateFineForPlayer(targetPlayerId)
    local input = lib.inputDialog('Issue Fine to Player: ' .. targetPlayerId, {
        {
            type = 'input',
            label = 'Fine Reason',
            description = 'Describe what the fine is for',
            placeholder = 'e.g. Illegal hunting',
            required = true,
            min = 3,
        },
        {
        ...
        },
        {
            ...
            required = true,
            options = {
                { value = 'newaustin',label = 'Tumbleweed'},
                { value = 'newaustin',label = 'Armadillo'},
                { value = 'newaustin',label = 'Blackwater'},
                { value = 'newhanover',label = 'Strawberry'},
                { value = 'newhanover',label = 'Valentine'},
                { value = 'lemoyne',label = 'Rhodes'},
                { value = 'lemoyne',label = 'Saint Denis'},
                { value = 'lemoyne',label = 'Annesburg'},
            }
        },
    })
    ...
    if input and input[1] and input[2] then
        ...
        if reason ~= '' and amount and amount > 0 then
            TriggerServerEvent('rsg-banking:server:applyCustomFine', targetPlayerId, reason, amount, location)
        else
            lib.notify({ title = 'Invalid input', type = 'error' })
        end
    end
end
