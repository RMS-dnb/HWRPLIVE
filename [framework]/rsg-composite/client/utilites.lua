...
function CreatePrompts()
	local str = CreateVarString(10, 'LITERAL_STRING', 'Take')
...
function ChangePlayerStats(health, stamina, stress, hunger, thirst, clean)
    ...
    -- Stress using FX-HUD
    if stress ~= 0 then
        exports['fx-hud']:setStatus("stress", stress)
    end

    -- Hunger & Thirst using FX-HUD
    if hunger ~= 0 then
        exports['fx-hud']:setStatus("hunger", hunger)
    end
    ...
    -- Cleanliness may not be supported by FX-HUD, so you can handle it custom
    if clean ~= 0 then
        -- Use your own system or remove this if unused
        TriggerServerEvent("RSGCore:Server:SetMetaData", "cleanliness", RSGCore.Functions.GetPlayerData().metadata["cleanliness"] + clean)
    end
end
