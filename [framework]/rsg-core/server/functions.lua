            ...
            if Player then
                ...
                if Player.PlayerData.job and payment > 0 and (RSGShared.Jobs[Player.PlayerData.job.name].offDutyPay or Player.PlayerData.job.onduty) then
                    if RSGCore.Config.Money.PayCheckSociety then
                        ...
                        MySQL.single.await('UPDATE players SET paycheck = paycheck + ? WHERE citizenid = ?', { payment, Player.PlayerData.citizenid })
                        TriggerClientEvent('ox_lib:notify', Player.PlayerData.source, {title = Lang:t('info.received_paycheck', { value = payment }), description = 'Go to your bank',type = 'info', duration = 5000 })
                    end
                end
            end
        end
...
function RSGCore.Functions.Notify(source, title, text, type, length)
    TriggerClientEvent('RSGCore:Notify', source, title, text, type, length)
end