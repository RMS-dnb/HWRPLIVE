...
local function ConvertCentsToDollars(player, centItem, dollarItem)
    local centItemData = player.Functions.GetItemByName(centItem)
    if centItemData and centItemData.amount >= 100 then
        local dollarsToAdd = math.floor(centItemData.amount / 100)
        local centsToRemove = dollarsToAdd * 100

        player.Functions.RemoveItem(centItem, centsToRemove, false)
        player.Functions.AddItem(dollarItem, dollarsToAdd, false)

        TriggerClientEvent("inventory:client:ItemBox", player.PlayerData.source, RSGCore.Shared.Items[centItem], "remove", centsToRemove)
        TriggerClientEvent("inventory:client:ItemBox", player.PlayerData.source, RSGCore.Shared.Items[dollarItem], "add", dollarsToAdd)
    end
end
...
function self.Functions.AddMoney(moneytype, amount, reason)
    ...
    if not self.Offline then
        ...
        local logAmount = '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') added, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason
        TriggerEvent('rsg-log:server:CreateLog', 'playermoney', 'AddMoney', 'lightgreen', logAmount, amount > 100000)
        ...
        -- Item money check
        if RSGCore.Config.Money.EnableMoneyItems then
            ConvertCentsToDollars(self, "cent", "dollar")
        end
    end
...
function self.Functions.RemoveMoney(moneytype, amount, reason)
    ...
    if not self.Offline then
        ...
        local logAmount = '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') removed, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason
        TriggerEvent('rsg-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'red', logAmount, amount > 100000)
