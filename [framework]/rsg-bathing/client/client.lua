    ...
    if Config.BathingZones[town] then
        ...
        while DoesCamExist(cam) do
            ...
            if IsControlPressed(0, `INPUT_CONTEXT_X`) and IsPromptEnabled("SCRUB") then
                ...
                while IsControlPressed(0, `INPUT_CONTEXT_X`) and IsPromptEnabled("SCRUB") do
    if IsPromptCompleted("SCRUB") then
        if DoesEntityExist(BathingPed) and not Config.BathingModes[bathMode].deluxe then
            bathMode = bathMode + 1
        end

        holdTime = holdTime + (Config.BathingModes[bathMode].hold_power or 0.25)

        -- << ADD THIS LINE TO INCREASE CLEANLINESS >>
       local currentClean = exports['fx-hud']:getStatus("cleanliness") or 0
        local newClean = math.min(currentClean + 0.85, 1.0)  -- increase by 0.1 per tick, adjust as needed
        exports['fx-hud']:setStatus("cleanliness", newClean)
