...
Citizen.CreateThread(function()
    ...
    if (Vdist(coords.x, coords.y, coords.z, -188.64, 718.24, 120.24) < 2.0) then
        if RSGCore.Functions.GetPlayerData().job.name == 'police' then -- Change 'pilot' to the job name allowed to use the balloon.
    ...
    if (Vdist(coords.x, coords.y, coords.z, -1369.349, -2357.067, 43.022) < 2.0) then
        if RSGCore.Functions.GetPlayerData().job.name == 'fisherman' then -- Change 'fisherman' to the job name allowed to use the rowboat.
...
RegisterCommand('clothingwheel', function()
