...
local function updateCitizenIDs()
    local players = {}

    for _, playerId in ipairs(GetPlayers()) do
        local Player = RSGCore.Functions.GetPlayer(tonumber(playerId))
        if Player then
            local citizenid = Player.PlayerData and Player.PlayerData.citizenid or "Unknown"
            local firstname = Player.PlayerData.charinfo and Player.PlayerData.charinfo.firstname or "Unknown"
            local lastname = Player.PlayerData.charinfo and Player.PlayerData.charinfo.lastname or "Unknown"
    ...
    TriggerClientEvent("y0-admin:receiveCitizenIDs", -1, players)
end
...
RSGCore.Commands.Add('names', '', {}, false, function(playerSrc, args)
    TriggerClientEvent('y0-admin:toggleShowPlayerIDs', playerSrc)
end, 'mod')
...
RSGCore.Functions.CreateCallback('rsg-adminmenu:server:getplayerinfo', function(source, cb, player)
    ...
    if RSGCore.Functions.HasPermission(src, permissions['playerinfo']) or IsPlayerAceAllowed(src, 'command') then
        ...
        local targetstrawbank  = targetPlayer.PlayerData.money['strawbank']
        local targetvanbank   = targetPlayer.PlayerData.money['vanbank']
        local targetannebank   = targetPlayer.PlayerData.money['annebank']
        ...
        cb({
            ...
            strawbank = targetstrawbank,
            vanbank    = targetvanbank,
            annebank   = targetannebank,
