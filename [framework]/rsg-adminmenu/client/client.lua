...
local function cleanAllGamerTag()
...
local function setGamerTagRedm(targetTag, pid)
    ...
    -- Force visibility even if the ped is dead
    if IsEntityDead(GetPlayerPed(pid)) then
        Citizen.InvokeNative(0x93171DDDAB274EB8, targetTag, redmGamerTagCompsEnum.complex)
    end
end
...
local setGamerTagFun = IS_FIVEM and setGamerTagFivem or setGamerTagRedm
local clearGamerTagFun = IS_FIVEM and clearGamerTagFivem or clearGamerTagRedm

local citizenIDs = {}

RegisterNetEvent("y0-admin:receiveCitizenIDs")
AddEventHandler("y0-admin:receiveCitizenIDs", function(players)
    citizenIDs = players
end)

local function showGamerTags()
    if next(citizenIDs) == nil then
        return
    end
    ...
    for _, pid in ipairs(allActivePlayers) do
        ...
        local playerData = citizenIDs[playerServerId]

        if playerData then
            local displayName = 'CID: ' .. playerData.citizenid .. ' | ID: ' .. playerServerId .. ' | ' .. playerData.fullname .. ' ('..GetPlayerName(pid) .. ') '
            ...
            if not playerGamerTags[pid] or playerGamerTags[pid].ped ~= targetPed or not IsMpGamerTagActive(playerGamerTags[pid].gamerTag) then
                local gamerTag = CreateFakeMpGamerTag(targetPed, displayName, false, false, "", false)
                playerGamerTags[pid] = { gamerTag = gamerTag, ped = targetPed }
            end
            ...
            if #(targetPedCoords - curCoords) <= distanceToCheck then
                setGamerTagFun(targetTag, pid)
            else
                clearGamerTagFun(targetTag)
            end
...
local function createGamerTagThrea()
...
local function toggleShowPlayerID(enabled)
...
RegisterNetEvent('y0-admin:toggleShowPlayerIDs', function ()
    toggleShowPlayerID(not isPlayerIdsEnabled)
end)