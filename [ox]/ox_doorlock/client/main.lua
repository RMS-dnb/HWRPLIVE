    ...
    while true do
        ...
        if num > 0 then
            for i = 1, num do
                ...
                -- make sure both values exist before comparing
                if door and door.distance and door.maxDistance then
                    if door.distance < door.maxDistance then
                        if not ClosestDoor or (ClosestDoor.distance and door.distance < ClosestDoor.distance) then
                            ClosestDoor = door
                        end
                    end
                else
                    print(('[ox_doorlock] Skipping invalid door entry: distance=%s maxDistance=%s'):format(
                        tostring(door and door.distance),
                        tostring(door and door.maxDistance)
                    ))
                end
            end
        end

        if ClosestDoor and ClosestDoor.distance and ClosestDoor.maxDistance and ClosestDoor.distance < ClosestDoor.maxDistance then
            ...
            if not PickingLock and IsControlJustReleased(0, 0x760A9C6F) then
                useClosestDoor()
            end
        elseif showUI then
            lib.hideTextUI()
