...
texts = Config.Texts[Config.Server.Language]
promptKey = Config.Prompt
DEBUG = Config.Debug
...
function printDebug(message)
    if DEBUG.print then
        if CLIENT then
            ...
            local debugPrefix = "\27[32m[DEBUG]\27[0m "
            print(debugPrefix .. message)
        end
    end
end

function printConfig(prefix,message)
    if DEBUG.print then
        local configPrefix = "\27[33m["..prefix.."]\27[0m "
        print(configPrefix .. message)
    end
end

function printInfo(prefix,message)
    local configPrefix = "\27[33m["..prefix.."]\27[0m "
    print(configPrefix .. message)
end

function printError(prefix, message)
    local errorPrefix = "\27[31m[ERROR]-["..prefix.."]\27[0m "
    print(errorPrefix .. message)
end


if SERVER then
    if DEBUG.print then 
        local warningMessage = "\27[31mDebug mode is enabled!\27[0m"
        printConfig('Debug',warningMessage)
    end

    if Config.Server.MetaTarget then
        printConfig('Target','is ON')
    end
end
