...
local function validateJob(item)
    if not item.jobs then return nil end

    local jobName = lib.callback.await('cw-crafting:getPlayerJobName', false)
    local jobType = lib.callback.await('cw-crafting:getPlayerJobType', false)
    ...
    for _, job in pairs(item.jobs) do
        ...
        if job.type and jobType and job.type == jobType then
            return true
        elseif job.name and job.name == jobName then
            return true
        end
    end
...
local function canCraftItem(item)
    ...
    for material, amount in pairs(item.materials) do
        ...
        if not hasItem(material, amount*CurrentAmount) then
            canCraft = false
        end
    end
    return canCraft
...
local function getRecipes()
    ...
    for recipe, item in pairs(Config.Recipes) do
        ...
        if validateRights(item, recipe) then
...
local function benchpermissions(jobTypes)
    local jobType = lib.callback.await('cw-crafting:getPlayerJobName', false)
...
RegisterNUICallback('attemptCrafting', function(recipe, cb)
    if isCrafting then
        lib.notify({
            description = "You're already crafting something",
            type = "error",
        })
    else
        ...
        lib.notify({
            description = "You can't craft a batch of 0.",
            type = "error",
        })
        ...
        return {
            {
                ...
                canInteract = function()
                    if benchType.jobName ~= nil then
                        return benchpermissions(benchType.jobName)
                    else
                        return true
                    end
                end,
