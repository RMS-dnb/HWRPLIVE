...
RegisterServerEvent('phils-bounty:server:sellnpcs', function(plate)
    ...
    if err then
        print("Debug: Wagon query error: ", err)
    ...
    if not npcs or #npcs == 0 then
        print("Debug: No NPCs found for plate ", plate)
        TriggerClientEvent('ox_lib:notify', src, { title = locale('sv_lang_31'), description = locale('sv_lang_32'), type = 'error', duration = 5000 })
        return
    end

    -- Calculate total sell price using Config.PricePerNPC
    local pricePerNPC = Config.PricePerNPC or 50 -- Fallback to 50 if not defined
    local totalPrice = #npcs * pricePerNPC
    print("Debug: Selling ", #npcs, " NPCs for plate ", plate, " at $", pricePerNPC, " each. Total price: $", totalPrice)

    -- Remove NPCs from inventory
    local success, err = MySQL.update('DELETE FROM `phils_bountywagon_inventory` WHERE plate = ?', { plate })
    if err then
        print("Debug: Delete NPCs error: ", err)
