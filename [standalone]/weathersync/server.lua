...
-- server.lua (FULL REPLACEMENT)
-- WeatherSync with time persistence (saves time every in-game minute)
-- Save file: resources/weathersync/sv_time.json

local ResourceName = GetCurrentResourceName()
local SaveFileName = "sv_time.json"

-- Config expected from config.lua
-- (config.lua is part of the resource and defines Config.time, Config.weather, etc.)
if not Config then
    print(("[^1%s^7] Warning: Config not loaded. Ensure config.lua is included in fxmanifest."):format(ResourceName))
    Config = Config or {}
end

-- Core state (copied from original)
local currentWeather = Config.weather
local currentTime = Config.time
local currentTimescale = Config.timescale
local weatherPattern = Config.weatherPattern
...
local function saveStateToFile(stateTable)
    local ok, encoded = pcall(json.encode, stateTable)
    if not ok then
        print(("[^1%s^7] Failed to json.encode state for saving."):format(ResourceName))
        return false
    end

    local saved = SaveResourceFile(ResourceName, SaveFileName, encoded, -1)
    if not saved then
        print(("[^1%s^7] Failed to SaveResourceFile('%s'). Ensure server has write permission."):format(ResourceName, SaveFileName))
    end
    return saved
end

local function loadStateFromFile()
    local content = LoadResourceFile(ResourceName, SaveFileName)
    ...
    if not ok then
        print(("[^1%s^7] Failed to parse %s, ignoring it."):format(ResourceName, SaveFileName))
        return nil
    end
...
local function SaveTimeToDisk()
    local state = {
        time = currentTime,
        savedAt = os.time()
    }
    local ok = saveStateToFile(state)
    if ok then
        -- optional debug:
        -- print(("[^2%s^7] Saved server time: %s"):format(ResourceName, tostring(currentTime)))
    end
    return ok
end

local function LoadTimeFromDisk()
    local parsed = loadStateFromFile()
    if parsed and parsed.time then
        -- expecting numeric seconds (time as week-seconds)
        if type(parsed.time) == "number" then
            currentTime = math.floor(parsed.time)
            print(("[^2%s^7] Loaded saved server time: %s"):format(ResourceName, tostring(currentTime)))
            return true
        else
            print(("[^1%s^7] sv_time.json time field invalid type, ignoring."):format(ResourceName))
            return false
        end
    end
    return false
...
-- Utility functions (original implementation preserved) --------------------
...
-- Uncommented admin commands are kept commented as original for server owners to enable
-- (left as in upstream code)

-- Persistence: keep track of last saved minute
local LastSavedMinute = nil

-- Load saved time on resource start (if present)
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName ~= ResourceName then return end

    -- Attempt to load saved time; fall back to Config.time if not present/invalid
    local loaded = LoadTimeFromDisk()
    if not loaded then
        currentTime = Config.time
        timeIsFrozen = Config.timeIsFrozen
        print(("[^3%s^7] No saved time; using Config.time."):format(ResourceName))
    else
        -- keep timeIsFrozen as configured (we are only persisting time, not frozen state)
        timeIsFrozen = Config.timeIsFrozen
    end

    -- Initialize LastSavedMinute to force a save immediately on next tick if needed
    local _, _, minute, _ = TimeToDHMS(currentTime)
    LastSavedMinute = (minute - 1) % 60
end)

-- Main tick loop (keeps original behavior, plus persistence hook)
Citizen.CreateThread(function()
    validateWeatherPattern(weatherPattern)
    ...
    while true do
        ...
        -- Save time to disk once the in-game minute changes
        do
            local d, h, m, s = TimeToDHMS(currentTime)
            if LastSavedMinute == nil or m ~= LastSavedMinute then
                LastSavedMinute = m
                SaveTimeToDisk()
            end
        end
...
-- Safe shutdown: attempt save on resource stop
AddEventHandler('onResourceStop', function(resourceName)
    if resourceName ~= ResourceName then return end
    SaveTimeToDisk()
end)

-- Manual save event (server console or other scripts can trigger)
RegisterNetEvent("weathersync:saveTime", function()
    SaveTimeToDisk()
end)

print(("[^2%s^7] server.lua loaded. Time persistence active (saving every in-game minute to %s)"):format(ResourceName, SaveFileName))
