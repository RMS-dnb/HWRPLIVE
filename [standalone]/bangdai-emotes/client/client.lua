...
lib.registerMenu({
    ...
    options = {
        ...
        {label = "Cancel Emote", description = "Stop current emote/animation", icon = 'fas fa-stop', args = {'cancel_emote'}},
    }
}, function(_, _, args)
    if args[1] == 'cari' then
        CloseMenu(true)
        ...
        if input then
            ...
            if #dpt < 1 then
                ...
                -- Add cancel option to search results
                table.insert(dpt, 1, {label = "Cancel Emote", description = "Stop current emote/animation", args = {"cancel"}})
                lib.registerMenu({
                    id = 'pencarian',
    ...
    elseif args[1] == 'cancel_emote' then
        CancelEmote()
        lib.showMenu('ip_animations', MenuIndexes['ip_animations'] or 1)
...
function GenerateWalkstyleMenu()
    ...
    optionsList[#optionsList + 1] = {
        label = "Cancel Emote",
        description = "Stop current emote/animation",
        args = {"cancel"}
    }
...
function GenerateEmoteMenu()
    lib.registerMenu({
        ...
        options = {
            {label = 'Cancel Emote', description = 'Stop current emote/animation'},
...
RegisterNetEvent("ip:emotes:client:menuscenario", function()
    ...
    table.sort(optionsList, function(a, b)
        if a.label == "Cancel Emote" then return true end
        if b.label == "Cancel Emote" then return false end
...
-- Add cancel command for completeness
RegisterCommand('cancel', function()
    CancelEmote()
end)

RegisterCommand('c e', function()
    CancelEmote()
end)
...
RegisterCommand('e', function(source, args)
    ...
    if emoteCommand == "cancel" or emoteCommand == "c" then
        CancelEmote()
        return
    end
