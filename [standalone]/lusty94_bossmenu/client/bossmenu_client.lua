...
function CLNotify(msg,type,time,title)
    ...
    if NotifyType == 'qb' then
        RSGCore.Functions.Notify(msg,type,time)
...
function OpenBossMenu(jobName)
    ...
    local jobLabel = RSGCore.Shared.Jobs[jobName].label
...
CreateThread(function()
    ...
    for jobName, coords in pairs(Config.Locations) do
        CLDebug(string.format('^3| Lusty94_BossMenu | DEBUG | INFO | Creating target zone for job: %s At coords: x=%.2f, y=%.2f, z=%.2f^7', jobName, coords.coords.x, coords.coords.y, coords.coords.z))

        if TargetType == 'qb' then
            exports['rsg-target']:AddCircleZone('bossMenu_'..jobName, coords.coords, 1.5, {
...
RegisterNetEvent('lusty94_bossmenu:client:HireEmployee', function(data)
    ...
    local gradeChoices = {}
    for grade, data in pairs(RSGCore.Shared.Jobs[job].grades) do
    ...
    local gradeLabel = RSGCore.Shared.Jobs[job].grades[tostring(selectedGrade)]?.name or ('Rank '..selectedGrade)
...
RegisterNetEvent('lusty94_bossmenu:client:PromoDemoEmployee', function(data)
    ...
    for _, v in pairs(members) do
        if v.citizenid ~= RSGCore.Functions.GetPlayerData().citizenid then
    ...
    local gradeLabel = RSGCore.Shared.Jobs[job].grades[tostring(newGrade)]?.name or ('Rank '..newGrade)
    local confirm = lib.alertDialog({
...
RegisterNetEvent('lusty94_bossmenu:client:FireEmployee', function(data)
    ...
    for _, m in pairs(members) do
        if m.citizenid ~= RSGCore.Functions.GetPlayerData().citizenid then
...
RegisterNetEvent('lusty94_bossmenu:client:SendMessage', function(data)
    ...
    local char = RSGCore.Functions.GetPlayerData().charinfo
    ...
    if resource == GetCurrentResourceName() then
        ...
        for _, zone in pairs(SpawnedZones) do
            if zone.type == 'qb' then
                exports['rsg-target']:RemoveZone(zone.name)
