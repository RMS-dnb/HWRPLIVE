...
RegisterCommand('hudload', function()
    -- Empty function cancels the original
end, false)
...
if Config.HudSettings.Hunger.decreaseOnRemoveHealthValue or Config.HudSettings.Thirst.decreaseOnRemoveHealthValue then
    CreateThread(function()
        while true do
            if logged then
                local ped = PlayerPedId()

                -- Prevent alerts and health reduction if the player is dead
                if IsEntityDead(ped) then
                    Wait(5000) -- Reduce CPU usage while dead
                else
                    -- Check Hunger
                    if hunger <= 0 and Config.HudSettings.Hunger.decreaseOnRemoveHealthValue then
                        hunger = 0
                        --Citizen.InvokeNative(0x4102732DF6B4005F, "MP_HealthDrop", 0, true)
                        --PlayPain(ped, 9, 1, true, true)
                        SetEntityHealth(ped, math.max(0, GetEntityHealth(ped) - Config.HudSettings.Hunger.decreaseOnRemoveHealthValue))
                    ...
                    -- Check Thirst
                    if thirst <= 0 and Config.HudSettings.Thirst.decreaseOnRemoveHealthValue then
                        thirst = 0
                        --Citizen.InvokeNative(0x4102732DF6B4005F, "MP_HealthDrop", 0, true)
                        --PlayPain(ped, 9, 1, true, true)
                        SetEntityHealth(ped, math.max(0, GetEntityHealth(ped) - Config.HudSettings.Thirst.decreaseOnRemoveHealthValue))
...
--     while true do
--         local sleep = true
--         local alcohol = local status = exports['fx-hud']:getStatus("alcohol")
--         if alcohol > 1 then
