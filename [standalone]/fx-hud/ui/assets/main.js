...
$(document).ready(function () {
    ...
    function resetHudData() {
        ...
        if (window.innerWidth >= 3440) {
            currentSizeData = {
                ...
                ".alchol": { "width": "1.8%", "height": "3.1%", "border-radius": "50%", "font-size": "1.5vh" },
    ...
    function setProgress(divname, value, autohide) {
        ...
        if (divname == ".level-bar") {
            ...
            $(divname + " .progress-bar").css({
                'background': `radial-gradient(closest-side, rgb(0, 0, 0) 79%, transparent 80% 100%), conic-gradient(${renk} ${value}%, rgb(37, 37, 37) 0)`
            });
        }
        ...
        //         if (divname == ".level-bar") {
        //             $(".level-bar").css({
        //                 'background': `linear-gradient(to right, ${renk} ${currentValue}%, transparent 0% 100%), linear-gradient(to right, rgb(0, 0, 0) 100%, transparent 80% 100%)`
        //             });
        //         } else {
        //             $(divname + " .progress-bar").css({
        //                 'background': `radial-gradient(closest-side, rgb(0, 0, 0) 79%, transparent 80% 100%), conic-gradient(${renk} ${currentValue}%, rgb(37, 37, 37) 0)`
        //             });
        //         }
    ...
    function changeProgressColor(div, deger) {
        let degisken = div.replace(" .progress-bar", "");
        hudColorData[div] = deger

        // // console.log(div)
        if (div == ".level-bar") {
            $(div).css({
                'background': `linear-gradient(to right, ${deger} 79%, transparent 80% 100%),
                linear-gradient(to right, rgb(0, 0, 0) 100%, transparent 80% 100%)`
            })
        } else {
            $(div).css({
                'background': `radial-gradient(closest-side, rgb(0, 0, 0) 79%, transparent 80% 100%),conic-gradient(${deger} ${currentHudData[degisken]}%, rgb(37, 37, 37) 0)`
            })
        }
    ...
    function showNotify(text, type, time) {
        ...
        $(".notify-container").append(`<div class="notify id${randomNumber}" style="display: none;">
    ...
    function openUIPanel() {
        ...
        for (let key in homesettings) {
            if (homeSettingsData[homesettings[key]] === 1) {
                $(homesettings[key]).fadeIn()
        ...
        $(".notify-container").append(`<div class="notify">
        <span class="notify-title">INFORM</span>
        <span class="notify-desc">You can change the location of this notification</span>
        <div class="notify-bar" role="progressbar" aria-valuenow="75" aria-valuemin="0" aria-valuemax="100">
        </div>
        <img id="notify-img" src="/ui/assets/img/info.png" alt="" srcset="">
    </div>`)
        // $(".ui-box").fadeIn();
    }
